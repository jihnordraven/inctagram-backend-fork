...
import { HttpExceptionFilter } from 'helpers/error-handlers'
...
describe('TESTING OF CREATING USER AND AUTH', () => {
	...
	it('should create user by super admin', async () => {
		...
		const allUsers = await request(server).get('/sa/users').set(authE2eSpec, basic)
		...
		const result = await request(server).get('/sa/users').set(authE2eSpec, basic)
	...
	it('shoud create user , ban it by SA and try to login => result must be 401', async () => {
		...
		const allUsers5 = await request(server).get('/sa/users').set(authE2eSpec, basic)
		...
		const allUsers4 = await request(server).get('/sa/users').set(authE2eSpec, basic)
		...
		const allUsers2 = await request(server).get('/sa/users').set(authE2eSpec, basic)
		...
		const allUsers3 = await request(server).get('/sa/users').set(authE2eSpec, basic)
	...
	it('shoud create user , create blog and try to delete blog', async () => {
		...
		await request(server).delete(`/blogger/blogs/${createdBlog.body.id}`).expect(401)
	})
	...
	it('shoud create user , create blog and ban blog sf', async () => {
		...
		const allBlogsBeforeBan = await request(server).get(`/blogs`).expect(200)
		...
		const allBlogsAfterBan = await request(server).get(`/blogs`).expect(200)

		expect(allBlogsAfterBan.body.items.length).toEqual(1)

		const BlogAfterBan = await request(server).get(`/blogs/${blogId}`).expect(404)
		...
		const getBlogById = await request(server).get(`/blogs/${blogId}`).expect(200)

		const allBlogsAfterBan2 = await request(server).get(`/blogs`).expect(200)
